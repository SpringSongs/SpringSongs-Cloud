...
Object.extend(String.prototype, {
  ...
  underscore: function() {
    return this.gsub(/::/, '/').gsub(/([A-Z]+)([A-Z][a-z])/, '#{1}_#{2}').gsub(/([a-z\d])([A-Z])/, '#{1}_#{2}').gsub(
      /-/, '_').toLowerCase();
  },
...
Element.Methods = {
  ...
  hasClassName: function(element, className) {
    ...
    if (elementClassName == className ||
      (typeof(elementClassName.match) === 'function' && elementClassName.match(new RegExp("(^|\\s)" + className +
        "(\\s|$)"))))
      return true;
    return false;
  },
...
Form.Methods = {
  ...
  findFirstElement: function(form) {
    return $(form).getElements().find(function(element) {
      return element.type != 'hidden' && !element.disabled && ['input', 'select', 'textarea'].include(element.tagName
        .toLowerCase());
    });
  },
