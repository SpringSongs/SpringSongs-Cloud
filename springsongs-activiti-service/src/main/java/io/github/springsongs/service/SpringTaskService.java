...
public class SpringTaskService {
	...
	public PageInfo<SpringTaskDTO> getTasksByStarter(String userId, String title, String category,int page,int size) {
		...
		histProcessInstanceQuery.listPage(page *size, size)
		...
		PageInfo SpringProcessPageInfo= new PageInfo<>(tasks);
		SpringProcessPageInfo.setList(tasks);
		return SpringProcessPageInfo;
	}

	public PageInfo<SpringTaskDTO> getTodoTasks(String userId, String title, String category,int page,int size) {
		List<SpringTaskDTO> tasks = new ArrayList<>();
		...
		List<SpringTaskDTO> taskDTOS = taskQuery
				.listPage(page * size, size).stream()
				.map(task -> {
	...
	public PageInfo<SpringTaskDTO> getFinishTasks(String userId, String title, String category,int page,int size) {
		List<SpringTaskDTO> tasks = new ArrayList<>();
		...
		List<SpringTaskDTO> taskDTOs = tasks.stream().sorted(Comparator.comparing(SpringTaskDTO::getTime).reversed())
				.collect(Collectors.toList());
		PageInfo SpringProcessPageInfo= new PageInfo<>(taskDTOs);
		SpringProcessPageInfo.setList(taskDTOs);
		return SpringProcessPageInfo;
	}
